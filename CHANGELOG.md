...
### Added
- **`render_chart` emits an interactive chart spec** (tagged `kind:"chart"`, with `type`/`title`/`data`/`xKey`/`yKeys`/`stacked`) for the custom Roads RAM UI to render as an interactive chart (bar/line/area/pie/scatter). The tool does no server-side plotting; the front-end draws it, so charts are interactive (hover tooltips, etc.) rather than static images.
- **`query_table` tool** — runs a single SQL `SELECT` and returns the result set as **structured JSON rows** (`columns` + `rows`), rather than the log/listing text `execute_sas_code` returns. This makes "show me the top N … as a chart" a single clean hop: `query_table` → `render_chart`. It runs the SELECT in a throwaway compute session (writing to `WORK._MCPQ`) and reads the result back through the Compute data API, so it works despite each call using a fresh session; on a SAS error it returns the log so the agent can correct the query. (`run_query_rows` / `fetch_session_table_rows` in `viya_utils.py`, tests in `tests/test_viya_utils.py` and `tests/test_tool_payloads.py`)
- **`get_ml_project_results` tool** — reads back an AutoML project's state, champion model, and model leaderboard with fit statistics (best-effort on the `/models` sub-resource), closing the "build a model → how good is it?" loop after `run_ml_project`.
- **Auto-scoping the data tools to the pinned dataset** — `get_castable_info`/`get_castable_columns`/`get_castable_data` now default their `server`/`caslib`/`table` to the primary `ALLOWED_TABLES` entry, so the agent calls them with no arguments and can't fumble identifiers. `get_use_case` now also returns the primary table's **columns**, grounding the agent immediately. New `DEFAULT_CAS_SERVER` (default `cas-shared-default`) and `DEFAULT_CASLIB` (default `Public`) env vars fill in coordinates a table entry omits. (`UseCaseScope.resolve` / `primary_table` in `usecase.py`)
- **`score_data` ergonomics** — `step_id` now defaults to `"score"` (the common case), and scoring is permitted for a module in either the models or decisions allowlist (`UseCaseScope.allows_scoreable`).
