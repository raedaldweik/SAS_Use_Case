...
def register_tools(mcp, get_token):
    ...
    async def render_chart(chart_type: str, title: str, data: list,
        ...
        The chart is drawn interactively by the custom front-end (the Roads RAM
        UI) from the spec this returns; the tool itself does no plotting. The
        spec is tagged ``kind: "chart"`` so the UI can detect and render it.
